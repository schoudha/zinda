...
interface TimeTabsProps {
  ...
  onValueChange: (value: "today" | "week" | "month" | "year") => void;
}

export const TimeTabs = memo(function TimeTabs({ value, onValueChange }: TimeTabsProps) {
  return (
      ...
      <Tabs value={value} onValueChange={(val) => onValueChange(val as "today" | "week" | "month" | "year")} className="w-full">
        <TabsList className="grid w-full grid-cols-4 bg-transparent p-0 gap-1 h-auto">
          <TabsTrigger
            value="today"
            className={cn(
              "rounded-full text-xs font-medium transition-all duration-300 py-2",
              value === "today"
